<template>
  <div class="pc-page">
    <div class="pc-page-header">
      <h1 class="pc-page-title">全局设置</h1>
      <button class="pc-primary-btn" @click="handleSave" :disabled="saving">
        <span class="material-symbols-outlined" style="font-size: 18px !important">save</span>
        {{ saving ? '保存中...' : '保存' }}
      </button>
    </div>
    ...
    <div class="pc-settings-grid">
      ...
      <div class="settings-card">
        ...
        <div class="form-group">
          ...
          <div class="input-with-icon">
            ...
            <input v-model="settings.github.token" type="password" placeholder="ghp_****************************" />
          </div>
      ...
      <div class="settings-card">
        ...
        <div class="form-group">
          ...
          <input v-model="settings.ozone.fetchCron" type="text" class="input-base" placeholder="留空表示不执行" />
          <p class="field-desc">从零零信安拉取数据的定时任务 Cron 表达式。留空不执行。</p>
        </div>
        <div class="form-group">
          <label>手动拉取测试</label>
          <button class="fetch-btn-mini" :class="{ fetching: fetching }" @click="handleManualFetch">
      ...
      <div class="settings-card">
        ...
        <div class="form-group">
          <label>定时拉取 (Cron)</label>
          <input v-model="settings.zoomeye.fetchCron" type="text" class="input-base" placeholder="留空表示不执行" />
          <p class="field-desc">触发 GitHub Action 的定时任务 Cron 表达式。留空不执行。</p>
        </div>
        <div class="form-group">
          <label>手动拉取测试</label>
          <button class="fetch-btn-mini" :class="{ fetching: zoomeyeFetching }" @click="handleZoomeyeManualFetch">
      ...
      <div class="settings-card">
        ...
        <div class="form-group">
          <label>定时拉取 (Cron)</label>
          <input v-model="settings.daydaymap.fetchCron" type="text" class="input-base" placeholder="留空表示不执行" />
          <p class="field-desc">从 DayDayMap 拉取数据的定时任务 Cron 表达式。留空不执行。</p>
        </div>
        <div class="form-group">
          <label>手动拉取测试</label>
          <button class="fetch-btn-mini" :class="{ fetching: daydaymapFetching }" @click="handleDaydaymapManualFetch">
      ...
      <div class="settings-card">
        ...
        <div class="form-group">
          ...
          <div class="input-with-icon">
            <span class="material-symbols-outlined input-prefix">key</span>
            <input v-model="settings.hunter.apiKey" type="password" placeholder="hunter API Key" />
          </div>
          <p class="field-desc">奇安信 Hunter API Key，用于解除速率限制。</p>
        </div>
        <div class="form-group">
          <label>定时拉取 (Cron)</label>
          <input v-model="settings.hunter.fetchCron" type="text" class="input-base" placeholder="留空表示不执行" />
          <p class="field-desc">从 Hunter 拉取数据的定时任务 Cron 表达式。留空不执行。</p>
        </div>
        <div class="form-group">
          <label>手动拉取测试</label>
          <button class="fetch-btn-mini" :class="{ fetching: hunterFetching }" @click="handleHunterManualFetch">
      ...
      <div class="settings-card">
        ...
        <div class="form-grid-2">
          ...
          <div class="form-group">
            <label>连接超时</label>
            <div class="input-with-unit">
              <input v-model.number="settings.engine.timeout" type="number" min="200" max="10000" step="100" />
        ...
        <div class="form-group">
          ...
          <div class="input-with-unit">
            <input v-model.number="settings.engine.configDelay" type="number" min="0" max="60" />
      ...
      <div class="settings-card">
        ...
        <div class="form-group">
          ...
          <input v-model="settings.scheduling.scanCron" type="text" class="input-base" placeholder="留空表示不执行" />
          <p class="field-desc">Cron 表达式：分 时 日 月 周。留空不执行。统一调度所有数据源扫描。</p>
        </div>
        <div class="form-group">
          <label>定时复测 (Cron)</label>
          <input v-model="settings.scheduling.janitorCron" type="text" class="input-base" placeholder="留空表示不执行" />
        ...
        <div class="cron-help">
          <details>
            ...
            <div class="help-content">
              ...
              <p><b>常用示例</b>：<code>0 2 * * *</code> 每天凌晨 2:00 | <code>*/30 * * * *</code> 每 30 分钟</p>
            </div>
          </details>
        </div>
      </div>
      ...
      <div class="settings-card">
        ...
        <div class="form-group">
          ...
          <input v-model="settings.security.callbackToken" type="text" class="input-base" placeholder="留空表示不启用认证" />
          <p class="field-desc">设置后，所有写操作需携带 X-Callback-Token 请求头。CF Worker 和 GitHub Action 需同步配置。</p>
        </div>
      </div>

      <!-- 后台日志入口 -->
      <div class="settings-card logs-entry-card" @click="$router.push('/pc/logs')">
        <div class="card-title-group">
...
const handleManualFetch = async () => {
  if (!settings.ozone.enabled) { toast.warning('请先启用 零零信安 数据源'); return }
  fetching.value = true
  try {
...
const handleZoomeyeManualFetch = async () => {
  if (!settings.zoomeye.enabled) { toast.warning('请先启用 ZoomEye 数据源'); return }
...
const handleHunterManualFetch = async () => {
  if (!settings.hunter.enabled) { toast.warning('请先启用 Hunter 数据源'); return }
...
const handleDaydaymapManualFetch = async () => {
  if (!settings.daydaymap.enabled) { toast.warning('请先启用 DayDayMap 数据源'); return }
...
.fetch-btn-mini:active { transform: scale(0.96); background: #0066D6; }
.fetch-btn-mini.fetching { opacity: 0.6; pointer-events: none; }
...
.help-content td { padding: 3px 8px; border-bottom: 1px solid #E8E8ED; }
...
.logs-entry-card:hover { box-shadow: var(--shadow-md); }
.entry-arrow { margin-left: auto; font-size: 22px !important; color: var(--text-muted); }
